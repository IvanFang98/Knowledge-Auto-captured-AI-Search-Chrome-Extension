...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function fetchNotebookLMNotebooks() {
    console.log('🌐 Preparing NotebookLM API request (MAIN world)...');

    // Use the GraphQL endpoint and query for listing notebooks
    ...
    try {
      ...
      console.warn('MAIN world fetch failed:', err.message);
    }
  ...
  async function addSourceToNotebook(notebookId, article) {
    ...
    // Try multiple API endpoints for adding sources
    const possibleEndpoints = [
      'https://notebooklm.google.com/_/NotebookLmGraphQl',
      'https://notebooklm.google.com/api/graphql',
      'https://notebooklm.google.com/_/api/graphql',
      'https://notebooklm.google.com/_/api/notebooklm/graphql'
    ];
    ...
    for (const endpoint of possibleEndpoints) {
      try {
        ...
        const mutation = `mutation AddExternalSource($notebookId: String!, $url: String!, $title: String, $text: String) { addExternalSource(notebookId: $notebookId, url: $url, title: $title, text: $text) { id title url } }`;
        const variables = {
          notebookId,
          url: article.url,
          title: article.title,
        ...
        const body = {
          operationName: 'AddExternalSource',
          variables,
          query: mutation
        };
        ...
        console.log(`📡 ${endpoint} - Status: ${res.status}, OK: ${res.ok}`);
        
        if (res.ok) {
          const result = await res.json();
          console.log('✅ Successfully added source via API');
          return result;
        }
      } catch (error) {
    ...
    throw new Error('Failed to add source: ' + article.title + ' (all API endpoints failed)');
  }
  ...
  async function fetchNotebookLMViaMainWorld(endpoint, fetchOptions) {
    return new Promise((resolve, reject) => {
      chrome.tabs.query({}, (tabs) => {
        const notebookTab = tabs.find(tab =>
          tab.url && tab.url.startsWith("https://notebooklm.google.com/")
        ...
        // Listen for the result (from content script relay)
        function handleResult(message, sender) {
          if (
            message.type === 'NOTEBOOKLM_API_RESULT_RELAY' &&
        ...
        // Inject fetch code into MAIN world
        chrome.scripting.executeScript({
          target: { tabId },
          world: 'MAIN',
          func: (endpoint, fetchOptions, requestId) => {
            fetch(endpoint, fetchOptions)
              .then(res => {
                console.log('MAIN world fetch status:', res.status);
                return res.json();
              })
              .then(data => {
                console.log('MAIN world fetch data:', data);
                window.postMessage({
                  type: 'NOTEBOOKLM_API_RESULT',
                  requestId,
                  data
                }, '*');
              })
          ...
          args: [endpoint, fetchOptions, requestId]
        });
      });
    });
  }
